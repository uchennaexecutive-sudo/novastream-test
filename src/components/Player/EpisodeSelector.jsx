...
function isUpcomingEpisode(ep) {
  const parsed = parseEpisodeDate(ep?.air_date)
...
function getEpisodeAvailabilityLabel(ep) {
  if (!isUpcomingEpisode(ep)) return ep?.air_date || ''

  const parsed = parseEpisodeDate(ep?.air_date)
  return parsed ? `Airs ${EPISODE_DATE_FORMATTER.format(parsed)}` : 'Not released yet'
}
...
export default function EpisodeSelector({
  ...
  const releasedSeasonEpisodes = episodes.filter((episode) => !isUpcomingEpisode(episode))
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        ...
        {onDownloadSeason && releasedSeasonEpisodes.length > 0 && (
          <motion.button
            onClick={() => onDownloadSeason(season, releasedSeasonEpisodes)}
        ...
        {loading ? (
          ...
          episodes.map((ep) => {
            ...
            return (
              <motion.div
                key={ep.id}
                className={`rounded-xl overflow-hidden group ${isUpcoming ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                style={{
                  background: 'var(--bg-surface)',
              ...
              >
                <div className="relative h-28">
                  {!isUpcoming && ep.still_path ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center" style={{ background: 'var(--bg-elevated)' }}>
                      <span className="text-base font-semibold opacity-55">{isUpcoming ? 'Soon' : 'TV'}</span>
                    </div>
                  )}
                  {!isUpcoming && (
                  ...
                  */}
                  {onDownloadEpisode && !isUpcoming && !isEpisodeDownloaded && (
                    <div
                ...
                <div className="p-3">
                  ...
                  {getEpisodeAvailabilityLabel(ep) && (
